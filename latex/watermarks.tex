...
Fig.~\ref{fig:changeJustOneLayer}. The variable are the entries of $W_\epsilon=[w_1,w_2,w_3,w_4]$ and the output of the network for input $V_1=(3,4)$ is:
\\
\begin{math}
...
\begin{subfigure}{0.5\linewidth}
    ...
    \tikzstyle{annot} = [text width=4em, text centered]
      ...
      \node[neuron node] (I-1) at (0,-1 cm) {$v_{1,1}$};
      \node[neuron node] (I-2) at (0,-2 cm) {$v_{1,2}$};
	  \node[neuron node] (I-3) at (0,-3 cm) {$v_{1,3}$};
      \node[neuron node] (I-4) at (0,-4 cm) {$v_{1,4}$};
      ...
      \path (I-1) edge[] node[below,xshift=0.5cm,yshift=-0.35cm] {$4$} (H-2);
      ...
      \path (I-4) edge[] node[above,xshift=0.5cm,yshift=0.35cm] {$3$} (H-1);
