...
\begin{definition}\textbf{DNN Distance.}
  ...
  $1\leq i \leq n^1$). We define the $p$-distance between $N^1$ and $N^2$,
  denoted $\norm{N^1-N^2}_p$, as:
  \[
    \norm{N^1-N^2}_p =    \bracketsR{\sum_{i=2}^{n^1}\sum_{j=1}^{s_{i}^1}\sum_{k=1}^{s_{i-1}^1}
    \abs{W^1_i[j,k] - W^2_i[j,k]}^p}^{1/p}
  \]
\end{definition}
Intuitively, this definition of distance compares the weight matrices
of the two networks, element-wise; it computes the $p$-norm
between each two corresponding edge weights as the distance between to vectors.
Fig.~\ref{fig:toyExampleModified} shows how these definitions are applied
to our toy example from Fig.~\ref{fig:toyExample}.
\\
\begin{math}
	\norm{N^1-N^2}_p=(\bracketsR{\abs{-0.5}^p+\abs{1}^p+\abs{1}^p+\abs{-0.5}^p+\abs{0.5}^p+\abs{0.5}^p)}^{1/p}\\
...
\begin{figure}[htp]
  ...
  \begin{subfigure}{0.5\linewidth}
    ...
    \tikzstyle{annot} = [text width=4em, text centered]
      ...
      % Connect every node in the hidden layer with the output layer
      \path (I-1) edge[] node[above,rotate=10] {$1.5$} (H-1);
      \path (I-1) edge[] node[below,rotate=-25] {$1$} (H-2);
      \path (I-2) edge[] node[above,rotate=25] {$-3$} (H-1);
      \path (I-2) edge[] node[below,rotate=-10] {$-0.5$} (H-2);
      \path (H-1) edge[] node[above,rotate=-10] {$0.5$} (O-1);
      \path (H-1) edge[] node[above,rotate=-25] {$-1.5$} (O-2);
      \path (H-2) edge[] node[below,rotate=25] {$-1$} (O-1);
      \path (H-2) edge[] node[below,rotate=10] {$1$} (O-2);
...
our running example from~\ref{fig:toyExample}. The constraint on the output of the network for input $V_1=(3,4)$ are a non-convex two-dimensional functions.
\\
\begin{math}
v_{3,1}=\bracketsR{(1+w_5)\cdot\relu{}((1+w_1)\cdot 3+(-2+w_3)\cdot 4)+(-1+w_7)\cdot\relu{}((2+w_2)\cdot 3+(-1+w_4)\cdot 4)}\\
v_{3,2}=\bracketsR{(1+w_6)\cdot\relu{}((1+w_1)\cdot 3+(-2+w_3)\cdot 4)+(-1+w_8)\cdot\relu{}((2+w_2)\cdot 3+(-1+w_4)\cdot 4)}
\end{math}
...
\begin{center}
    ...
    \tikzstyle{annot} = [text width=4em, text centered]
      ...
      \path (H-1) edge[] node[above,rotate=-10] {$1+w_5$} (O-1);
      \path (H-1) edge[] node[above,rotate=-25] {$-1+w_6$} (O-2);
      \path (H-2) edge[] node[below,rotate=25] {$-1+w_7$} (O-1);
      \path (H-2) edge[] node[below,rotate=10] {$1+w_8$} (O-2);
...
In addition, if the distance metric in use is the $\infty$-norm,
...
distance, we used the $1$-norm and $\infty$-norm, which are popular
...
ran the experiment once with $1$-norm, and once with $\infty$-norm; the
...
others; specifically, in the $\infty$-norm case, the more resilient
watermarks were approximately $26\%$ more difficult to remove than
others, and in the $1$-norm case --- about $150\%$ more
...
distance metrics used, i.e. $1$-norm and $\infty$-norm. Not surprisingly,
the distances in the $1$-norm case are generally larger than those in the
$\infty$-norm case: this is because the $\infty$-norm allows us to
change multiple edges ``at no additional cost'', and thus effect the
network's outcome more effectively; whereas $1$-norm counts
every edge weight that changes. In
Fig.~\ref{fig:lastLayerExampleSingle} we visualize this on one
of the watermarks; we see that indeed, when using the $\infty$-norm,
almost all the edges were touched, whereas the $1$-norm caused the
...
\centering
  ...
  \includegraphics[width=\linewidth]{../data/results/problem3/last_layer_1_wm_example.pdf}
     \caption{The Minimal change according to $\infty$-norm for a single input}
  \end{subfigure}
  \begin{subfigure}{0.4\linewidth}
    \includegraphics[width=\linewidth]{../data/results/problem2/last_layer_1_wm_example.pdf}
    \caption{The Minimal change according to $1$-norm for a single input}
  \end{subfigure}
...
vary widely between $1$-norm and $\infty$-norm. We observe that using 
$1$-norm gives better accuracy result on average, but can lead to poor
accuracy in extreme cases; whereas the $\infty$-norm leads to a much
...
output layer when using the $\infty$-norm is significantly easier
than other variants; and indeed, using Gurobi as our backend
verification tool we were able to find modifications
that remove as many as all of the watermarks. In the $1$-norm case,
...
that, at least in the $\infty$-norm case, removing all watermarks renders
...
\caption{Change and accuracy when solving for minimal $\infty$-norm change.}
\end{subtable}
\begin{subtable}{1\textwidth}
\centering
\resizebox{\linewidth}{!}{
...
\caption{Change and accuracy when solving for minimal $1$-norm change.}
\end{subtable}
