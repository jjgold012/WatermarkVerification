...
\begin{figure}[htp]
  \begin{center}
    ...
    \tikzstyle{annot} = [text width=4em, text centered]
      ...
      \path (I-1) edge[] node[below,rotate=-20] {$2$} (H-2);
      \path (I-2) edge[] node[above,rotate=20] {$-2$} (H-1);
      \path (I-2) edge[] node[below,rotate=-10] {$-1$} (H-2);
      \path (H-1) edge[] node[above,rotate=-10] {$1$} (O-1);
      \path (H-2) edge[] node[below,rotate=20] {$-1$} (O-1);
      \path (H-1) edge[] node[above,rotate=-20] {$-1$} (O-2);
...
\begin{figure}[htp]
  \begin{subfigure}{0.5\linewidth}
    ...
    \tikzstyle{annot} = [text width=4em, text centered]
      ...
      % Annotate the layers
      \node[annot,above of=H-1, node distance=1cm] (l) {$V^1_2$};
      \node[annot,left of=l] {$V^1_1$};
      \node[annot,right of=l] {$V^1_3$};
    \end{tikzpicture}
    \caption{$N^1$}
  \end{subfigure}
  \begin{subfigure}{0.5\linewidth}
    ...
    \tikzstyle{annot} = [text width=4em, text centered]
      ...
      \path (I-1) edge[] node[below,rotate=-20] {$1$} (H-2);
      \path (I-2) edge[] node[above,rotate=20] {$-3$} (H-1);
      \path (I-2) edge[] node[below,rotate=-10] {$-0.5$} (H-2);
      \path (H-1) edge[] node[above,rotate=-10] {$0.5$} (O-1);
      \path (H-2) edge[] node[below,rotate=20] {$-1$} (O-1);
      \path (H-1) edge[] node[above,rotate=-20] {$-1.5$} (O-2);
      \path (H-2) edge[] node[below,rotate=10] {$1$} (O-2);
      
      % Annotate the layers
      \node[annot,above of=H-1, node distance=1cm] (l) {$V^2_2$};
      \node[annot,left of=l] {$V^2_1$};
      \node[annot,right of=l] {$V^2_3$};
    \end{tikzpicture}
    \caption{$N^2$}
  \end{subfigure}
