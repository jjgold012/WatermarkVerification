...
\begin{abstract}
  ...
  ways that affects certain aspects of its behavior, while leaving
  ...
  retraining the DNN is difficult and expensive, and may produce a new
  ...
  thus less likely to
  affect other aspects of the DNN. Using a proof-of-concept
...
for this paradigm is that training modern DNNs often requiring
...
These examples, and others, share the same main characteristics: an
existing DNN needs to be changed in a small way, that changes some
behaviors and keeps other unchanged. Existing approaches for
...
counter-example. Here, we show how to reduce the problem of making
changes to the DNN itself into a DNN verification problem. Specifically,
we propose a method for constructing a new DNN, and then pose a
verification query on that DNN that is satisfiable if and only if the
original DNN can be modified in the desired way. Due to the soundness
of the underlying DNN verification techniques that we use, our
technique affords formal guarantees about the modified DNN ---
...
We show how verification-based technique can be used to
measure the resilience of watermarks within a DNN. Our approach can be
used to assess individual DNNs before their release, and also can be
used to compare and rank different watermarking schemes, according to
...
We show how our approach can be used as part of a loop, in which
minimal modifications to a DNN are performed in order to correct
erroneous behaviors, and the DNN is then verified to ensure that the
desired result has been obtained. If the erroneous behavior is not
eliminated immediately, we add the newly discovered counter-example as
a constraint, and find a new set of minimal modification that correct
it, as well.
...
our running example from~\ref{fig:toyExample}. The constraint on the output of the network for input $V_1=(3,4)$ are a non-convex two-degree functions.
\\
\begin{math}
